...
export interface TreeDataGridRCGProps<R, SR = unknown, K extends Key = Key>
  extends Omit<
...
export function TreeDataGridRCG<R, SR = unknown, K extends Key = Key>({
  columns: rawColumns,
  rows: rawRows,
  ...
  const { columns, groupBy } = useMemo(() => {
    const groupBy: string[] = [...rawGroupBy];
    const columns = [...rawColumns];

    // Add a new synthetic column at the beginning to show group values
    if (groupBy.length > 0) {
      columns.unshift({
        key: '__group__',
        name: 'Group',
